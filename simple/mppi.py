...
class MPPIPlanner(ABC):
    ...
    def __init__(self, cfg: MPPIConfig, nx: int, dynamics: Callable,
                 running_cost: Callable, prior: Optional[Callable] = None):
        ...
        assert all([len(cfg.noise_sigma[0]) == len(row)
                   for row in cfg.noise_sigma])
        ...
        # Make sure if any of the input limits are specified, both are
        # specified
        ...
        self.nx = nx
        self.nu = 1 if len(
            self.noise_sigma.shape) == 0 else self.noise_sigma.shape[0]

        # Moments and best trajectory
        self.mean_action = torch.zeros(
            self.nu, device=self.tensor_args['device'],
        ...
        self.gamma_seq = torch.cumprod(
            torch.tensor([1.0] + [self.gamma] * (self.T - 1)),
            dim=0).reshape(
            1, self.T)
    ...
    def get_samples(self, sample_shape, **kwargs):
        ...
        if (self.sample_method == 'halton'):
            self.knot_points = generate_gaussian_halton_samples(
                ...
                # n_knots * nu (knots per number of actions)
                self.ndims,
                use_ghalton=True,
            ...
            knot_samples = self.knot_points.view(
                sample_shape, self.nu, self.n_knots)  # n knots is T/knot_scale (30/4 = 7)
            self.samples = torch.zeros(
                (sample_shape, self.T, self.nu),
                **self.tensor_args)
            for i in range(sample_shape):
                for j in range(self.nu):
                    self.samples[i, :, j] = bspline(
                        knot_samples[i, j, :], n=self.T, degree=self.degree)
    ...
    def _compute_rollout_costs(self, perturbed_actions):
        ...
        cost_total = torch.zeros(
            K, device=self.tensor_args['device'],
            dtype=self.tensor_args['dtype'])
        cost_horizon = torch.zeros(
            [K, T],
            device=self.tensor_args['device'],
            dtype=self.tensor_args['dtype'])
        cost_samples = cost_total

        # allow propagation of a sample of states (ex. to carry a
        # distribution), or to start with a single state
        ...
        for t in range(T):
            ...
            # Update action if there were changes in fusion mppi due for
            # instance to suction constraints
    ...
    def _update_distribution(self, costs, actions):
        ...
        if self.update_cov:
            ...
            self.cov_action = (
                1.0 - self.step_size_cov) * self.cov_action + self.step_size_cov * cov_update
    ...
    def get_action_cost(self):
        if self.noise_abs_cost:
            action_cost = self.lambda_ * torch.abs(
                self.noise) @ self.noise_sigma_inv
    ...
    def _compute_total_cost_batch_simple(self):
        ...
        # Bounded noise after bounding (some got cut off, so we don't penalize
        # that in action cost)
    ...
    def _compute_total_cost_batch_halton(self):
        ...
        scaled_delta = torch.matmul(
            self.delta, torch.diag(
                self.scale_tril)).view(
            self.delta.shape[0], self.T, self.nu)
        ...
        perturbation_cost = torch.sum(
            self.mean_action * action_cost, dim=(1, 2))
        # self.cost_total += perturbation_cost
        return self.cost_total
