...
def run_panda_robot(cfg: ExampleConfig):
    ...
    while n_trials < cfg.n_steps:
        ...
        if count > 96:
            ...
            Ex, Ey, Etheta = client_helper.compute_metrics(
                block_pos, block_ort)
            ...
            if baseline == 1:
                ...
                # Ex < 0.02 and Ey < 0.02 and Etheta < 0.1  -- Success for
                # baseline 1
            ...
            print(
                f"FPS: {1/(time.time() - t)} RT-factor: {cfg.isaacgym.dt/(time.time() - t)}")
