...
class Objective(object):
    def __init__(self, cfg, device):
        ...
        self.block_goal_pose_emdn_0 = torch.tensor(
            [0.5, 0.3, 0.5, 0.0, 0.0, 0.0, 1.0],
            device=cfg.mppi.device)
        self.block_goal_pose_emdn_1 = torch.tensor(
            [0.4, 0.3, 0.5, 0, 0, -0.7071068, 0.7071068],
            device=cfg.mppi.device)  # Rotation 90 deg

        self.block_goal_pose_ur5_c = torch.tensor(
            [0.65, 0, 0.5, 0, 0, 0, 1], device=cfg.mppi.device)
        self.block_goal_pose_ur5_l = torch.tensor(
            [0.7, 0.2, 0.5, 0, 0, 0.258819, 0.9659258],
            device=cfg.mppi.device)  # Rotation 30 deg
        self.block_goal_pose_ur5_r = torch.tensor(
            [0.7, -0.2, 0.5, 0, 0, -0.258819, 0.9659258],
            device=cfg.mppi.device)  # Rotation -30 deg
        ...
        self.goal_yaw = torch.atan2(
            2.0 *
            (self.block_ort_goal[-1] * self.block_ort_goal[2] + self.
             block_ort_goal[0] * self.block_ort_goal[1]),
            self.block_ort_goal[-1] * self.block_ort_goal[-1] + self.
            block_ort_goal[0] * self.block_ort_goal[0] - self.block_ort_goal
            [1] * self.block_ort_goal[1] - self.block_ort_goal[2] * self.
            block_ort_goal[2])
    ...
    def compute_metrics(self, block_pos, block_ort):
        ...
        block_yaws = torch.atan2(
            2.0 *
            (block_ort[:, -1] * block_ort[:, 2] + block_ort[:, 0] *
             block_ort[:, 1]),
            block_ort[:, -1] * block_ort[:, -1] + block_ort[:, 0] *
            block_ort[:, 0] - block_ort[:, 1] * block_ort[:, 1] -
            block_ort[:, 2] * block_ort[:, 2])
    ...
    def compute_cost(self, sim):
        ...
        robot_euler = pytorch3d.transforms.matrix_to_euler_angles(
            pytorch3d.transforms.quaternion_to_matrix(r_ort), "ZYX")
        ...
        # Posture costs
        ee_align = torch.linalg.norm(
            robot_euler[:, 0:2] - self.ort_goal_euler[0:2], axis=1)
        ee_hover_dist = torch.abs(ee_height - self.ee_hover_height)
        push_align = torch.sum(
            robot_to_block[:, 0: 2] * block_to_goal, 1) / (
            robot_to_block_dist * block_to_goal_pos) + 1

        # print(push_align[-1])
        # Collision avoidance
        xyz_contatcs = torch.reshape(
            (torch.sum(torch.abs(sim.net_cf), dim=1)), (sim.num_envs, sim.num_bodies))
